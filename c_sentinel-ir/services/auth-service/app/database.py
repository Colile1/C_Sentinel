"""
database.py - auth-service's own database engine, session factory and probe.

This service holds exactly one connection string, its own `auth_db`, and never
another service's - the Database-per-Service pattern claimed for the mark. The
engine is built lazily so the process can start and answer /health/live before
auth-db is up, and `database_reachable` is what /health calls to report whether
it answers.
...
# Consul polls /health every 10s; a check that blocks on a dead database for the
# driver's default timeout would pile requests up. Fail fast instead.
...
def get_engine() -> Engine:
    """
    Purpose: the single SQLAlchemy engine for auth-db, built once per process.
    Inputs:  none; the connection string comes from this service's settings.
    Output:  a SQLAlchemy Engine with `pool_pre_ping`, so connections the
             database closed underneath us are dropped rather than handed out.
    """
...
def init_database() -> None:
    ...
    try:
        ...
        _logger.info("Database schema ready: auth_db")
    except Exception as exc:  # noqa: BLE001 - startup must not crash-loop
        _logger.warning("Could not prepare auth_db schema: %s", exc)


def database_reachable() -> bool:
    """
    Purpose: report whether auth-db answers, for the /health endpoint.
    Inputs:  none.
    ...
    try:
        ...
        _logger.warning("auth-db is unreachable: %s", exc)
        return False
