"""
test_skeleton.py - the build-step-4 verification for asset-service.
...
/metrics returns Prometheus exposition, the correlation-id middleware echoes the
header, and unknown routes come back in the one error shape.
...
def test_health_live_ignores_the_database(make_app) -> None:
    ...
    assert response.status_code == 200
    assert response.json() == {"status": "alive", "service": "asset-service"}
...
def test_unknown_route_uses_the_shared_error_shape(client: TestClient) -> None:
    ...
    response = client.get("/api/v1/assets/does-not-exist-yet")
    assert response.status_code == 404
    body = response.json()
    assert set(body) == {"error", "message", "details"}


def test_openapi_advertises_health_but_not_metrics(client: TestClient) -> None:
    """/health is part of the documented API; /metrics and /health/live are not."""
