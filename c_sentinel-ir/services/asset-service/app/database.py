"""
database.py - asset-service's own database engine, session factory and probe.

This service holds exactly one connection string, its own `asset_db`, and
...
/health/live) even when asset-db is not yet up, and `database_reachable` is
what /health calls to report whether asset-db answers.
...
def init_database() -> None:
    ...
    try:
        ...
        _logger.info("Database schema ready: asset_db")
    except Exception as exc:  # noqa: BLE001 - startup must not crash-loop
        _logger.warning("Could not prepare asset_db schema: %s", exc)
